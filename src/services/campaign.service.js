...
const createCampaign = async (data, files, deviceIds, sectorIds) => {
    const { name, parsedStartDate, parsedEndDate, company_id, media_metadata } = data;
        ...
        if (files && files.length > 0) {
            for (const [index, file] of files.entries()) {
                ...
                await client.query(
                    ...
                    [newCampaign.id, file.originalname, filePath, file.mimetype, metadata.order, metadata.duration]
                );
            }
        }

        for (const device_id of deviceIds) {
            await client.query(`INSERT INTO campaign_device (campaign_id, device_id) VALUES ($1, $2)`, [newCampaign.id, device_id]);
        }
        for (const sector_id of sectorIds) {
            await client.query(`INSERT INTO campaign_sector (campaign_id, sector_id) VALUES ($1, $2)`, [newCampaign.id, sector_id]);
        }
...
const deleteCampaign = async (id) => {
    ...
    try {
        await client.query("BEGIN");
        const uploadsResult = await db.query("SELECT file_path FROM campaign_uploads WHERE campaign_id = $1", [id]);
        
        await client.query("DELETE FROM campaign_device WHERE campaign_id = $1", [id]);
        await client.query("DELETE FROM campaign_sector WHERE campaign_id = $1", [id]);
        await client.query("DELETE FROM campaign_uploads WHERE campaign_id = $1", [id]);
        const deleteResult = await client.query("DELETE FROM campaigns WHERE id = $1", [id]);
